...
추가로, 제안기법이 실제로 어느 구간에서 손해를 보는지를 분리해서 보기 위해 보조 검증을 따로 수행하였다. 이 보조 검증은 네 가지 경우를 비교한다. `baseline`은 기본 실행 방식, `geometry_only`는 기하 정보 재사용만 켠 경우, `full_legacy`는 현재 full pair 실행 전체, `full_no_expand`는 full 실행에서 불필요한 edge 확장을 줄인 실험 버전이다. 측정 모드는 두 가지다. `forward_energy`는 입력 그래프를 미리 만든 뒤 에너지만 계산하므로 순수 forward 경로를 본다. `step_force`는 그래프 생성, pair metadata 생성, device 이동, 모델 실행, 힘 계산까지 포함하므로 실제 MD 한 step에 더 가깝다. 이 보조 검증은 원인 분석을 위한 실험이므로, 본문의 주 성능 결과와 같은 표에서 직접 섞지 않는다.
...
보조 분리 검증은 더 안정적인 통계를 위해 warmup 5회 후 100회 반복 측정하였다. 따라서 표 6부터 표 9까지의 값은 모두 `100회 반복의 평균 ± 표준편차`에 기반하며, 제안기법의 구조적 문제를 진단하는 데 사용한다.
...
### 5.4 forward-only와 force 포함 경로의 분리 검증

앞 절까지의 결과는 `SevenNet 기본 실행 방식`과 `제안기법`의 실제 추론 시간을 보여주지만, 왜 어떤 데이터셋에서는 좋아지고 어떤 데이터셋에서는 나빠지는지까지 바로 설명해 주지는 않는다. 이를 확인하기 위해 우리는 제안기법을 더 잘게 나누어 다시 측정하였다. 첫째, `geometry_only`는 기하 정보 재사용만 켠 경우이다. 둘째, `full_legacy`는 현재 full pair 실행 전체이다. 셋째, `full_no_expand`는 full 실행에서 `EDGE_ATTR/EDGE_EMBEDDING`의 불필요한 확장을 줄인 실험 버전이다. 이 세 경우를 기본 실행 방식과 비교해 순수 forward 경로와 force 포함 경로를 따로 측정하였다.

표 6과 표 7은 각각 `step_force`와 `forward_energy` 결과를 정리한 것이다. 가장 먼저 확인된 점은, pure forward에서는 `geometry_only`가 거의 본전에 가깝다는 사실이다. 31개 데이터셋 전체에서 `baseline / geometry_only`의 중앙값은 `0.977배`였다. 반면 같은 조건에서 `full_legacy`와 `full_no_expand`는 각각 `0.865배`, `0.862배`로 더 느렸다. 즉, 현재 구현에서 큰 손해를 만드는 핵심은 단순한 기하 정보 재사용 자체가 아니라, full 실행이 메시지 계산을 둘로 나누고 pair 상태를 끝까지 유지하지 못하는 구조에 더 가깝다.

하지만 force를 포함한 `step_force`로 넘어가면 상황이 더 나빠진다. `baseline / geometry_only`의 중앙값은 `0.684배`, `baseline / full_no_expand`의 중앙값은 `0.694배`였다. `geometry_only`의 경우 pure forward에서는 거의 본전인데, force 포함 경로에서는 크게 느려진다는 뜻이다. 이 차이는 제안기법의 forward 자체보다, 힘 계산을 위해 뒤에서 다시 도는 backward 경로와 step 바깥 오버헤드가 더 크게 작용함을 보여준다. 따라서 “MD step 전체가 느리다”는 관찰만으로는 제안기법 자체의 가능성을 바로 판단할 수 없고, forward-only와 force 포함 경로를 반드시 분리해 보아야 한다.

또 하나 중요한 점은 `full_no_expand`의 효과가 매우 작았다는 것이다. `full_legacy / full_no_expand`의 중앙값은 pure forward에서 `0.996배`, force 포함 경로에서 `0.998배`였다. 이는 `EDGE_ATTR/EDGE_EMBEDDING` 확장 제거만으로는 현재 full path의 구조적 손해를 해소할 수 없음을 뜻한다. 다시 말해, 지금 필요한 것은 작은 패치 하나가 아니라 pair 상태를 더 오래 유지하는 실행 구조 자체의 재설계다.

![그림 8](../figures/pair_validation_split/pair_validation_forward_energy_latency_all.png)

![그림 9](../figures/pair_validation_split/pair_validation_step_force_latency_all.png)

표 8은 pair metadata 생성 방식도 별도로 비교한 결과이다. 현재 Python/CPU 기반 구현의 중앙값은 `55.866 ms`였고, 같은 논리를 벡터화한 CPU 실험은 `10.019 ms`, GPU kernel-only 실험은 `0.730 ms`였다. 즉, 현재 pair metadata 경로는 실제로 무시할 수 없는 오버헤드이며, 이 부분만 벡터화하거나 device로 옮겨도 구조적으로 줄일 여지가 크다. 이 결과는 현재 손해가 “pair 재사용 아이디어 자체”의 한계라기보다, 런타임 구현 방식의 한계에서 상당 부분 온다는 점을 뒷받침한다.

![그림 10](../figures/pair_validation_split/pair_validation_pair_metadata_methods.png)

Representative `torch.profiler` 결과도 같은 해석을 지지한다. 작은 그래프 `qm9_hf`와 큰 그래프 `mptrj`를 보면, pure forward에서는 `aten::tensordot`, `aten::mm`, `aten::einsum`과 같은 일반 convolution 경로가 중심이다. 반면 force를 포함하면 `autograd::engine::evaluate_function`, `SliceBackward0`, `aten::slice_backward`, `aten::add_`와 같은 backward 관련 연산이 상위에 올라온다. 즉, SevenNet의 힘 계산은 마지막 MLP만 다시 미분하는 것이 아니라, 전체 에너지를 `EDGE_VEC`에 대해 미분하면서 상류의 convolution, `weight_nn`, geometric path를 함께 통과한다. 이 때문에 forward-only와 force 포함 결과가 크게 다르게 나온다.

### 5.5 구간별 시간 분석과 오버헤드 해석
...
![그림 11](../figures/figure_04_representative_stage_breakdown.png)
