...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for spec, inventory_row in selected_specs:
        ...
        line = f"[pair-accuracy] {sample.dataset} :: {sample.sample_id} :: natoms={sample.natoms}"
    ...
    raw_df = _write_csv(global_dir / "pair_accuracy_raw_repeats.csv", raw_rows)
    ...
    write_json(
        global_dir / "environment.json",
        {
            **gpu_info(),
            "warmup": args.warmup,
            "repeat": args.repeat,
            "cases": [case["case"] for case in PAIR_CASES],
        },
    )

    summary_text = "\n".join(
        [
            "# KCC Pair Accuracy Repeats",
            "",
            f"- Datasets benchmarked: `{summary_df['dataset'].nunique()}`",
            ...
            "- `metrics/pair_accuracy/per_dataset/<dataset>/pair_accuracy_summary.csv`",
        ]
    )
    (output_root / "summary_pair_accuracy.md").write_text(summary_text + "\n")
    return 0
