...
def _write_per_dataset(
    *,
    ...
    _write_csv(dataset_dir / "reference_e3nn.csv", [row for row in reference_rows if row["dataset"] == dataset])
    ...
    _write_csv(dataset_dir / "pair_end_to_end_raw_repeats.csv", dataset_raw)
    _write_csv(dataset_dir / "pair_end_to_end_summary.csv", dataset_summary)
    if not dataset_summary.empty:
        pivot_mean = dataset_summary.pivot(index="dataset", columns="case", values="mean_ms")
        pivot_std = dataset_summary.pivot(index="dataset", columns="case", values="std_ms")
        if {"e3nn_baseline", "e3nn_pair_full"}.issubset(pivot_mean.columns):
            cmp_df = pd.DataFrame(
                [
                    {
                        "dataset": dataset,
                        "baseline_mean_ms": float(pivot_mean.loc[dataset, "e3nn_baseline"]),
                        "baseline_std_ms": float(pivot_std.loc[dataset, "e3nn_baseline"]),
                        "proposal_mean_ms": float(pivot_mean.loc[dataset, "e3nn_pair_full"]),
                        "proposal_std_ms": float(pivot_std.loc[dataset, "e3nn_pair_full"]),
                        "speedup_baseline_over_proposal": float(
                            pivot_mean.loc[dataset, "e3nn_baseline"] / pivot_mean.loc[dataset, "e3nn_pair_full"]
                        ),
                    }
                ]
            )
            _write_csv(dataset_dir / "pair_end_to_end_comparison.csv", cmp_df)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for spec, inventory_row in selected_specs:
        ...
        line = f"[pair-end-to-end] {sample.dataset} :: {sample.sample_id} :: natoms={sample.natoms}"
        ...
        _write_csv(dataset_dir / "reference_e3nn.csv", [reference_row])
            ...
            summary_rows.append(
                {
                    ...
                    "max_abs_force_diff_vs_e3nn": float(np.max(np.abs(np.asarray(result["forces"]) - ref["forces"]))),
                }
            )
    ...
    _write_csv(global_dir / "reference_e3nn.csv", reference_rows)

    summary_mean_std_df = aggregate_numeric(
        raw_df,
        keys=("dataset", "sample_id", "modal", "natoms", "case", "n_warmup", "n_repeat", "device", "resolved_policy"),
    )
    _write_csv(global_dir / "pair_end_to_end_raw_mean_std.csv", summary_mean_std_df)
    ...
    summary_text = "\n".join(
        [
            ...
            "- `metrics/pair_end_to_end/per_dataset/<dataset>/pair_end_to_end_summary.csv`",
        ]
    )
    (output_root / "summary_pair_end_to_end.md").write_text(summary_text + "\n")
    return 0
