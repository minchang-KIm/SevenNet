...
intrusive forward-only 진단은 다른 결론을 준다.  
이 측정은 headline latency가 아니라 남은 구조 손해를 보기 위한 stage comparison이다.
...
- `bulk_large`
  ...
  - baseline / geometry_only: `1.023x`
- `bulk_small`
  - baseline forward total: `2.980 ms`
  - geometry_only forward total: `3.059 ms`
  - baseline / geometry_only: `0.974x`
- `dimer_small`
  - baseline forward total: `2.812 ms`
  - geometry_only forward total: `2.962 ms`
  - baseline / geometry_only: `0.949x`

남아 있는 대표 비용:

- pair expand: 약 `0.038 ~ 0.041 ms`
- pair weight expand: 약 `0.033 ~ 0.037 ms`
- pair geometry: 약 `0.179 ~ 0.192 ms`

이 결과는 다음을 뜻한다.

1. geometry-side exact reuse 자체가 수학적으로 틀렸거나 무의미해서 느린 것은 아니다.
2. large/dense forward diagnostic에서는 이미 거의 본전 또는 약한 이득이 보인다.
3. end-to-end generic calculator path에서 손해를 만드는 큰 축은 geometry 계산 그 자체보다 step 바깥과 pair->edge 확장 구조다.
...
- legacy pair_metadata: `4.612 ± 0.052 ms`
- upstream pair_metadata: `0.304 ± 0.014 ms`
- reduction: `15.19x`
- geometry_only total: `170.320 ± 7.123 ms -> 167.296 ± 7.276 ms`
- total reduction: `1.018x`

`bulk_small`

- legacy pair_metadata: `0.439 ± 0.013 ms`
- upstream pair_metadata: `0.101 ± 0.008 ms`
- reduction: `4.35x`
- geometry_only total: `168.856 ± 13.518 ms -> 162.871 ± 4.693 ms`
- total reduction: `1.037x`

이 결과는 두 가지를 말해준다.

1. pair metadata는 실제로 줄일 가치가 큰 구조 병목이었다.
2. upstream에서 pair 정보를 직접 쓰게 만들면 이 병목은 꽤 확실하게 줄어든다.
...
3. 그러나 intrusive forward diagnostic과 LAMMPS upstream pair-metadata bench는, 병목이 exact reuse 수식 자체가 아니라 runtime 구조에 있음을 보여준다.
...
4. **actionable next-step evidence**
