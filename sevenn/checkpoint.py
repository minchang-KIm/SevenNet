...
class SevenNetCheckpoint:
    ...
    def __repr__(self) -> str:
        ...
        dct = {
            ...
            'SWIFT-TP used': self.config.get(KEY.USE_SWIFT_TP, False),
        }
    ...
    def build_model(
        ...
        *,
        ...
        enable_swift: Optional[bool] = None,
        ...
        cp_using_swift = self.config.get(KEY.USE_SWIFT_TP, False)
        enable_swift = cp_using_swift if enable_swift is None else enable_swift

        cp_using_oeq = self.config.get(KEY.USE_OEQ, False)
        enable_oeq = cp_using_oeq if enable_oeq is None else enable_oeq

        if sum([enable_cueq, enable_flash, enable_swift, enable_oeq]) > 1:
        ...
        cfg_new[KEY.USE_SWIFT_TP] = enable_swift
        cfg_new[KEY.USE_OEQ] = enable_oeq

        if (cp_using_cueq, cp_using_flash, cp_using_swift, cp_using_oeq) == (
            enable_cueq,
            enable_flash,
            enable_swift,
            enable_oeq,
