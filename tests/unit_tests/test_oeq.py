...
def test_model_output(cf, use_pairaware):
    ...
    oeq_out = model_oeq._preprocess(get_graphs(batched=True))

    for k, e3nn_f in model_e3nn._modules.items():
        oeq_f = model_oeq._modules[k]
        e3nn_out = e3nn_f(e3nn_out)  # type: ignore
        oeq_out = oeq_f(oeq_out)  # type: ignore
        assert torch.allclose(e3nn_out.x, oeq_out.x, atol=1e-6), (
            f'{k} \n\n {e3nn_f} \n\n {oeq_f}'
        )

    assert torch.allclose(
        e3nn_out.inferred_total_energy, oeq_out.inferred_total_energy
    )
    assert torch.allclose(e3nn_out.atomic_energy, oeq_out.atomic_energy)
    assert torch.allclose(
        e3nn_out.inferred_force, oeq_out.inferred_force, atol=1e-5
    )
    assert torch.allclose(
        e3nn_out.inferred_stress, oeq_out.inferred_stress, atol=1e-5
    )
...
def test_checkpoint_convert(tmp_path, start_from_oeq):
    torch.manual_seed(123)
    model_from = get_model(use_oeq=start_from_oeq)

    cfg = get_model_config()
    cfg.update(
        {
            'use_oeq': start_from_oeq,
    ...
    model_to, _ = model_from_checkpoint(
        str(tmp_path / 'cp_from.pth'), enable_oeq=(not start_from_oeq)
    )
    model_to.to('cuda')
...
def test_checkpoint_convert_no_batch(tmp_path, start_from_oeq):
    torch.manual_seed(123)
    model_from = get_model(use_oeq=start_from_oeq)
...
def test_calculator(tmp_path):
    ...
    cfg = get_model_config()
    cfg.update(
        {'use_oeq': oeq, 'version': sevenn.__version__}
    )
    ...
    calc2 = SevenNetCalculator(cp_path, enable_oeq=False)
