"""Audit local IsoDelta-Halo goal readiness from the current source tree.

The audit is intentionally dependency-free and evidence-oriented. It does not
claim that remote authentication works or that a heavy 8-GPU LAMMPS job has
already run; instead it verifies that the local implementation, validation
scripts, sync gate, paper-suite tooling, documentation, and CI workflow are
present and mutually connected.
"""

from __future__ import annotations

import argparse
import ast
import io
import json
from pathlib import Path
import subprocess
import time
import tokenize
from typing import Any


# These constants make the audit criteria explicit rather than hiding them in
# ad-hoc string checks.
REPO_ROOT_PARENT_DEPTH = 1
REPO_ROOT = Path(__file__).resolve().parents[REPO_ROOT_PARENT_DEPTH]
GOAL_READINESS_SCHEMA_VERSION = "isodelta-goal-readiness-v1"
GENERATED_REPORT_COMMENT_KEY = "report_comment"
GOAL_READINESS_REPORT_COMMENT = (
    "IsoDelta-Halo local goal-readiness report auditing source markers, "
    "generated-file comments, forbidden temporary-work notes, branch identity, "
    "and validation-gate wiring for the active implementation goal."
)
SUCCESS_RETURN_CODE = 0
FAILURE_RETURN_CODE = 1
GIT_METADATA_TIMEOUT_SECONDS = 10.0
STATUS_PASSED = "passed"
STATUS_FAILED = "failed"
STATUS_NOT_ENFORCED = "not_enforced"
EXPECTED_BRANCH = "codex/isodelta-halo-runtime"
PYTHON_HEADER_PREFIX = '"""'
ISODELTA_PYTHON_GLOB_PATTERNS = (
    "tools/*isodelta*.py",
    "tests/unit_tests/test_isodelta*.py",
)
ISODELTA_PRODUCTION_GLOB_PATTERNS = (
    "tools/*isodelta*.py",
    "sevenn/pair_e3gnn/pair_e3gnn_parallel.cpp",
    "sevenn/pair_e3gnn/pair_e3gnn_parallel.h",
    "sevenn/pair_e3gnn/comm_brick.cpp",
    "sevenn/pair_e3gnn/comm_brick.h",
    "sevenn/pair_e3gnn/pair_e3gnn.cpp",
    "sevenn/pair_e3gnn/patch_lammps.sh",
)
FORBIDDEN_IMPLEMENTATION_MARKERS = (
    "TODO",
    "FIXME",
    "HACK",
    "XXX",
    "not implemented",
    "stub",
    "placeholder",
    "temporary implementation",
    "magic number",
    "hardcoded",
)
FORBIDDEN_FILE_SNIPPETS = {
    "sevenn/pair_e3gnn/pair_e3gnn_parallel.cpp": (
        "[6]",
        "(1024 * 1024)",
        "x_dim * n * 4",
        "std::getenv(kIsoDeltaHaloDisableEnv) == nullptr",
        "std::getenv(kIsoDeltaHaloProfileEnv) != nullptr",
        "send size(MB)",
        "int tag_to_graph_idx[natoms + 1]",
        "int graph_index_to_i[ntotal]",
        "float edge_vec[nedges_upper_bound][3]",
        "long edge_idx_src[nedges_upper_bound]",
        "long edge_idx_dst[nedges_upper_bound]",
        "const int jtag = tag[j];\n      j &= NEIGHMASK;",
        "graph_idx != -1",
        "idx == -1",
        "int n_model = std::stoi(arg[2]);",
        "std::filesystem::exists(arg[3])",
        "int chem_arg_i = 4",
        "for (int i = 3; i < n_model + 3; i++)",
        "chem_arg_i = n_model + 3",
        'cutoff = std::stod(meta_dict["cutoff"])',
        'int comm_size = std::stod(meta_dict["comm_size"])',
        "std::strtok",
        "const_cast<char *>",
        'auto delim = " "',
    ),
    "sevenn/pair_e3gnn/pair_e3gnn_parallel.h": (
        "[6]",
        "static constexpr const char *",
    ),
    "sevenn/pair_e3gnn/comm_brick.cpp": (
        "int all[6]",
        "MPI_Allreduce(&recvneed[0][0],all,6",
        "nswap > 6",
    ),
    "sevenn/pair_e3gnn/pair_e3gnn.cpp": (
        'cutoff = std::stod(meta_dict["cutoff"])',
        "std::strtok",
        "const_cast<char *>",
        'auto delim = " "',
        "for (int i = 3; i < narg; i++)",
        "ntypes > narg - 3",
    ),
    "sevenn/scripts/deploy.py": (
        "chem_list.strip()",
        "chem_list += chemical_symbols",
    ),
}
REQUIRED_FILE_SNIPPETS = {
    "sevenn/pair_e3gnn/pair_e3gnn_parallel.cpp": (
        "IsoDelta-Halo",
        "SEVENN_ISODELTA_HALO_DISABLE",
        "SEVENN_ISODELTA_HALO_PROFILE",
        "iso_delta_halo_env_flag_is_enabled",
        "normalize_iso_delta_halo_env_flag_value",
        "std::isspace(static_cast<unsigned char>(*begin))",
        "std::isspace(static_cast<unsigned char>(*last_character))",
        'kIsoDeltaHaloEnvFlagValueZero = "0"',
        'kIsoDeltaHaloEnvFlagValueFalse = "false"',
        'kIsoDeltaHaloEnvFlagValueNo = "no"',
        'kIsoDeltaHaloEnvFlagValueOff = "off"',
        "comm_cache_miss_reason_is_valid",
        "kSpatialDimension = 3",
        "kVoigtStressComponentCount = 6",
        "kAtomTagIndexBase = 1",
        "kInvalidGraphIndex = -1",
        "std::vector<int> tag_to_graph_idx",
        "std::vector<int> graph_index_to_i",
        "std::vector<float> edge_vec_storage",
        "std::vector<long> edge_idx_src",
        "std::vector<long> edge_idx_dst",
        "tag_to_graph_idx.data()",
        "graph_index_to_i.data()",
        "kIsoDeltaHaloEdgeBufferSizeError",
        "checked_graph_buffer_index",
        "checked_edge_buffer_capacity",
        "checked_edge_buffer_index",
        "checked_edge_storage_element_count",
        "checked_edge_storage_offset",
        "kIsoDeltaHaloAtomTypeMapError",
        "initialize_atom_type_map",
        "checked_lammps_atom_type",
        "checked_model_atom_type",
        "kUnmappedAtomType",
        "kIsoDeltaHaloAtomTagLookupSizeError",
        "kIsoDeltaHaloAtomTagIndexError",
        "checked_atom_tag_lookup_size",
        "checked_atom_tag_index",
        "checked_atom_tag_graph_index",
        "j &= NEIGHMASK;\n      validate_comm_atom_index(j, atom->nmax, error);\n      const tagint jtag = tag[j];",
        "kIsoDeltaHaloGraphIndexRequiredError",
        "tag_to_graph_idx_ptr = nullptr;",
        "if (tag_to_graph_idx_ptr == nullptr)",
        "graph_idx != kInvalidGraphIndex",
        "kIsoDeltaHaloCommPhaseRangeError",
        "validate_comm_phase(int comm_phase) const",
        "comm_phase >= kCommPhaseCount",
        "idx == kInvalidGraphIndex",
        "kIsoDeltaHaloCommInitCountError",
        "kIsoDeltaHaloCommInitRangeError",
        "kIsoDeltaHaloSendListRequiredError",
        "checked_comm_init_count",
        "checked_comm_init_last_index",
        "validate_comm_atom_index",
        "kIsoDeltaHaloExtraGraphIndexError",
        "kIsoDeltaHaloExtraTensorSizeError",
        "checked_extra_graph_index",
        "checked_extra_tensor_size",
        "kIsoDeltaHaloNodeFeatureShapeError",
        "checked_node_feature_width",
        "kNodeFeatureTensorRank",
        "kNodeFeatureWidthDimension",
        "kIsoDeltaHaloGraphAtomIndexError",
        "checked_graph_atom_index",
        "checked_graph_index_capacity",
        "check_cuda_status(cudaError_t cuda_err",
        "kCudaSendBufferAllocationError",
        "kCudaRecvBufferAllocationError",
        "kCudaSendBufferFreeError",
        "kCudaRecvBufferFreeError",
        "kCudaBufferSizeError",
        "kCudaDeviceCountError",
        "kCudaSetDeviceError",
        "kCudaPackForwardMemcpyError",
        "kCudaPackReverseMemcpyError",
        "cudaGetErrorString(cuda_err)",
        "checked_cuda_buffer_byte_count",
        "const torch::Device &target_device",
        "index_tensor.scalar_type() == torch::kInt64",
        "index_tensor.device() == target_device",
        "kE3GnnPayloadElementCountError",
        "checked_e3gnn_payload_element_count",
        "checked_e3gnn_payload_byte_count",
        "payload_byte_count",
        "return payload_element_count;",
        "kPairCoeffArgumentError",
        "kPairCoeffNumericMetadataError",
        "kPairCoeffSpeciesMetadataError",
        "kPairCoeffWildcardFirstIndex = 0",
        "kPairCoeffWildcardSecondIndex = 1",
        "kPairCoeffModelCountIndex = 2",
        "kPairCoeffModelPathIndex = 3",
        "kPairCoeffDirectorySpeciesStartIndex = 4",
        "kPairCoeffExplicitModelStartIndex = 3",
        "kMinimumPairCoeffArgumentCount = 5",
        "kMinimumModelFileCount = 1",
        "validate_pair_coeff_minimum_args",
        "checked_parse_positive_int",
        "checked_parse_positive_double_metadata",
        "checked_parse_positive_int_metadata",
        "checked_explicit_model_species_start",
        "parse_chemical_symbol_tokens",
        "validate_deployed_species_metadata",
        "validate_pair_coeff_species_count",
        "std::istringstream symbol_stream(chemical_symbols)",
        "!std::isfinite(parsed_value)",
        "validate_pair_coeff_minimum_args(narg, error);",
        "checked_parse_positive_int(arg[kPairCoeffModelCountIndex], error)",
        "std::filesystem::exists(arg[kPairCoeffModelPathIndex])",
        "chem_arg_i = checked_explicit_model_species_start(n_model, narg, error);",
        "for (int i = kPairCoeffExplicitModelStartIndex; i < chem_arg_i; i++)",
        'cutoff = checked_parse_positive_double_metadata(meta_dict["cutoff"], error);',
        'checked_parse_positive_int_metadata(meta_dict["comm_size"], error)',
        'checked_parse_positive_int_metadata(meta_dict["num_species"], error)',
        "validate_deployed_species_metadata(deployed_species_count, chem_vec.size()",
        "if (n_chem <= kMinimumGraphNodeCount)",
        "validate_pair_coeff_species_count(n_chem, ntypes, error);",
        "for (size_t j = 0; j < chem_vec.size(); j++)",
        "static_cast<int>(j); // LAMMPS atom types are 1-based.",
    ),
    "sevenn/pair_e3gnn/pair_e3gnn_parallel.h": (
        "IsoDelta",
        "metadata",
        "void get_buffer(int, int, float *&, float *&, class Error *)",
    ),
    "sevenn/pair_e3gnn/comm_brick.cpp": (
        "kE3GnnCommPhaseLimit = 6",
        "kE3GnnCommPhaseRangeError",
        "kE3GnnSendlistIndexRangeError",
        "void CommBrick::validate_e3gnn_comm_phase(int iswap) const",
        "iswap >= nswap",
        "iswap >= kE3GnnCommPhaseLimit",
        "index >= sendnum[iswap]",
        "validate_e3gnn_sendlist_index(iswap, index);",
        "checked_e3gnn_buffer_elements",
        "kE3GnnFeatureWidthError",
        "kE3GnnBufferCapacityError",
        "std::numeric_limits<int>::max()",
        "std::vector<float> host_send_buffer",
        "std::vector<float> host_recv_buffer",
        "e3gnn_forward_send_capacity",
        "e3gnn_reverse_recv_capacity",
        "buf_recv_, error);",
    ),
    "sevenn/pair_e3gnn/comm_brick.h": (
        "validate_e3gnn_comm_phase(int iswap) const",
        "validate_e3gnn_sendlist_index(int iswap, int index) const",
    ),
    "sevenn/pair_e3gnn/pair_e3gnn.cpp": (
        "kSerialPairCoeffArgumentError",
        "kSerialPairCoeffNumericMetadataError",
        "kSerialPairCoeffSpeciesMetadataError",
        "kSerialPairCoeffWildcardFirstIndex = 0",
        "kSerialPairCoeffWildcardSecondIndex = 1",
        "kSerialPairCoeffModelPathIndex = 2",
        "kSerialPairCoeffSpeciesStartIndex = 3",
        "kMinimumSerialPairCoeffArgumentCount = 4",
        "kMinimumSerialSpeciesCount = 1",
        "kSerialFirstLammpsAtomType = 1",
        "validate_serial_pair_coeff_minimum_args(narg, error);",
        "checked_serial_positive_double_metadata",
        "checked_serial_positive_int_metadata",
        "parse_serial_chemical_symbol_tokens",
        "validate_serial_deployed_species_metadata",
        "validate_serial_pair_coeff_species_count",
        "std::istringstream symbol_stream(chemical_symbols)",
        "!std::isfinite(parsed_value)",
        "torch::jit::load(std::string(arg[kSerialPairCoeffModelPathIndex])",
        'cutoff = checked_serial_positive_double_metadata(meta_dict["cutoff"], error);',
        'checked_serial_positive_int_metadata(meta_dict["num_species"], error)',
        "validate_serial_pair_coeff_species_count(n_chem, ntypes, error);",
        "for (size_t j = 0; j < chem_vec.size(); j++)",
        "map[lammps_atom_type] = static_cast<int>(j);",
        "if (lmp->logfile)",
    ),
    "sevenn/scripts/deploy.py": (
        "def _chemical_symbols_to_index_metadata",
        "Mapping[int, int]",
        "' '.join(",
        "chemical_symbols[atomic_number] for atomic_number in type_map.keys()",
        "chem_list = _chemical_symbols_to_index_metadata(type_map)",
    ),
    "tools/check_isodelta_build_prereqs.py": (
        "PREREQ_REPORT_SCHEMA_VERSION",
        "REPORT_SCHEMA_VERSION_KEY",
        "PREREQ_REPORT_COMMENT",
        "GENERATED_REPORT_COMMENT_KEY",
        "build_prereq_report",
        "REPORT_SCHEMA_VERSION_KEY: PREREQ_REPORT_SCHEMA_VERSION",
        "GENERATED_REPORT_COMMENT_KEY: PREREQ_REPORT_COMMENT",
    ),
    "tools/check_isodelta_lammps_binary.py": (
        "BINARY_SMOKE_REPORT_SCHEMA_VERSION",
        "REPORT_SCHEMA_VERSION_KEY",
        "BINARY_SMOKE_REPORT_COMMENT",
        "GENERATED_REPORT_COMMENT_KEY",
        "build_binary_check_report",
        "REPORT_SCHEMA_VERSION_KEY: BINARY_SMOKE_REPORT_SCHEMA_VERSION",
        "GENERATED_REPORT_COMMENT_KEY: BINARY_SMOKE_REPORT_COMMENT",
    ),
    "tools/run_isodelta_cluster_paper_suite.py": (
        "--prepare-artifacts",
        "--preflight-only",
        "--pipeline",
        "--readiness-check",
        "--verify-output-bundle",
        "--verify-pipeline-report",
        "verify_pipeline_report",
        "FINAL_PAPER_READINESS_CHECK_NAMES",
        "PIPELINE_READINESS_CHECKS_ERROR",
        "_require_pipeline_readiness_report",
        "PIPELINE_ARTIFACT_PREPARATION_ARTIFACTS_ERROR",
        "_require_pipeline_artifact_preparation_report",
        "PIPELINE_PLAN_MODES_ERROR",
        "PIPELINE_PLAN_CASES_ERROR",
        "PIPELINE_PLAN_REQUIRED_PAPER_OUTPUT_KEYS",
        "PIPELINE_PLAN_OUTPUT_ALIGNMENT_ERROR",
        "_require_pipeline_plan_report",
        "_require_pipeline_plan_output_alignment",
        "_require_pipeline_summary_stage_alignment",
        "verified_summary_stage_count",
        "_require_pipeline_summary_suite_report",
        "verified_suite_field_count",
        "verified_paper_output_count",
        "PAPER_ARTIFACT_COMMENTS",
        "GENERATED_ARTIFACT_COMMENT_KEY",
        "READINESS_REPORT_COMMENT",
        "ARTIFACT_PREPARATION_REPORT_COMMENT",
        "PREFLIGHT_REPORT_COMMENT",
        "EXTERNAL_TIMING_REPORT_COMMENT",
        "GENERATED_REPORT_COMMENT_KEY: EXTERNAL_TIMING_REPORT_COMMENT",
        "PIPELINE_REPORT_COMMENT",
        "OUTPUT_BUNDLE_VERIFICATION_COMMENT",
        "RUN_PLAN_REPORT_COMMENT",
        "PIPELINE_STAGE_REPORT_PATH_ALIGNMENT_ERROR",
        "PIPELINE_SUMMARY_STAGE_ALIGNMENT_ERROR",
        "PIPELINE_SUMMARY_SUITE_ERROR",
        "PIPELINE_SUMMARY_SUITE_ALIGNMENT_KEYS",
        "PIPELINE_ARTIFACT_PLAN_ALIGNMENT_ERROR",
        "PIPELINE_PREFLIGHT_ARTIFACT_ALIGNMENT_ERROR",
        "PIPELINE_PREFLIGHT_SUITE_ERROR",
        "PIPELINE_STAGE_SUITE_MANIFEST_ALIGNMENT_KEYS",
        "PIPELINE_STAGE_SUITE_ALIGNMENT_KEYS",
        "_require_suite_manifest_alignment",
        "_require_stage_suite_metadata_alignment",
        "_require_pipeline_artifact_plan_alignment",
        "_require_pipeline_preflight_artifact_alignment",
        "verified_preflight_artifact_alignment_count",
        "summary.suite.manifest.path",
        "suite.manifest.path",
        "summary.suite.manifest.size_bytes",
        "suite.manifest.size_bytes",
        "readiness_report.suite.manifest",
        "artifact_preparation_report.suite.manifest",
        "preflight_report.suite.manifest",
        "run_plan.suite.manifest",
        "summary.suite.min_trace_count",
        "suite.min_trace_count",
        "summary.suite.min_distinct_trace_models",
        "suite.min_distinct_trace_models",
        "PIPELINE_SUITE_ARTIFACT_SHA_ERROR",
        "SUMMARY_PIPELINE_STAGE_NAMES",
        "stage report fingerprint path",
        "_require_report_comment",
        "_require_csv_artifact_comment",
        "_require_markdown_artifact_comment",
        "_svg_desc_content",
        "_validate_external_timing_command_records",
        "_expected_external_timing_command_names",
        "_external_timing_role_from_command_name",
        "unexpected external timing command records",
        "_require_external_command_log_fingerprints",
        "_require_external_timing_reports_from_summary",
        "COMMAND_LOG_FINGERPRINTS_KEY",
        "COMMAND_LOG_SIDECAR_SCHEMA_VERSION",
        "COMMAND_LOG_SIDECAR_COMMENT",
        "COMMAND_LOG_SIDECAR_KEY",
        "command_log_sidecar_path",
        "_write_command_log_sidecar",
        "command_log_fingerprint(",
        "_require_command_log_sidecar",
        "command_log_sidecar_schema_version",
        "command log sidecar must exist",
        "EXPERIMENT_REPORT_CHECK_NAME",
        "EXPERIMENT_REPORT_CHECK_PATH",
        "EXPERIMENT_REPORT_KEY",
        "EXPERIMENT_REPORT_CHECK_KEY",
        "_validate_experiment_report_check",
        "_require_experiment_report_checks_from_summary",
        "verified_experiment_report_check_count",
        "experiment-report-check",
        "verified_external_command_log_count",
        "PREFLIGHT_ENVIRONMENT_SNAPSHOT_ARTIFACT_KEY",
        "PIPELINE_PREFLIGHT_ENVIRONMENT_SNAPSHOT_BUNDLE_ERROR",
        "_require_optional_preflight_environment_snapshot_semantics",
        "_require_pipeline_preflight_summary_snapshot_alignment",
        "verified_preflight_environment_snapshot_count",
        "verified_preflight_environment_snapshot_bundle_count",
        "OUTPUT_BUNDLE_VERIFICATION_COUNT_KEYS",
        "must match current bundle verification",
        "PIPELINE_BUNDLE_SUMMARY_PATH_ALIGNMENT_ERROR",
        "summary_path_candidates",
        "REQUIRED_PAPER_ARTIFACT_NAMES",
        "_require_artifact_index_alignment",
        "verified_artifact_index_count",
        "_require_case_summary_cell_values",
        "_summary_correlations_by_metric_pair",
        "_format_csv_value",
        "PAPER_COMMAND_TIMING_COLUMNS",
        "_command_timing_rows",
        "_require_command_timing_csv",
        "_require_command_timing_markdown",
        "command_timing_csv",
        "PAPER_REPEAT_TIMING_COLUMNS",
        "_repeat_timing_rows",
        "_repeat_timing_rows_from_summary",
        "_require_repeat_timing_csv",
        "_require_repeat_timing_markdown",
        "repeat_timing_csv",
        "PAPER_SPEEDUP_UNCERTAINTY_COLUMNS",
        "_speedup_uncertainty_rows",
        "_speedup_uncertainty_rows_from_summary",
        "_require_speedup_uncertainty_csv",
        "_require_speedup_uncertainty_markdown",
        "PAPER_REQUIRED_MODEL_COVERAGE_COLUMNS",
        "MODEL_COVERAGE_STATUS_COVERED",
        "_required_model_coverage_rows_from_case_records",
        "_required_model_coverage_rows_from_summary",
        "_require_required_model_coverage_csv",
        "_require_required_model_coverage_markdown",
        "required_model_coverage.csv",
        "required_model_coverage.md",
        "passed/reused trace",
        "_case_record_trace_evidence_paths",
        "trace_check.validate_trace_evidence",
        "paths must match cases.",
        "invalid trace evidence",
        "_require_required_trace_model_labels",
        "trace evidence is missing required model labels",
        "required_trace_models",
        "suite_evidence",
        "_suite_evidence_from_summary",
        "_require_suite_evidence_alignment",
        "verified_suite_evidence_count",
        "SUITE_EVIDENCE_THRESHOLD_ORIGIN_VERIFICATION_COUNT",
        "suite_evidence.{field_name} must match passed summary cases",
        "summary.suite trace thresholds are required when",
        "suite_evidence.min_trace_count must match summary.suite.min_trace_count",
        "suite_evidence.min_distinct_trace_models must match",
        "suite_evidence.trace_evidence_count must satisfy",
        "suite_evidence.distinct_trace_model_count must satisfy",
        "_correlation_rows_from_summary",
        "summary correlations.",
        "must match passed cases",
        "_require_paper_artifact_semantics",
        "verified_paper_artifact_semantic_count",
        "_manifest_snapshot_body",
        "manifest_snapshot",
        "manifest_snapshot: body SHA-256 must match suite.manifest.sha256",
        "_require_speedup_svg_semantics",
        "_require_speedup_uncertainty_svg_semantics",
        "_require_summary_speedup_measurements",
        "_summary_required_models",
        "_summary_case_has_passing_status",
        "_summary_passing_measured_speedup_case_names",
        "_summary_passing_speedup_ci_case_names",
        "_summary_passing_numeric_pair_case_names",
        "_case_summary_has_passing_status",
        "_paper_claim_case_summaries",
        "_summary_models_for_cases",
        "_require_required_model_speedup_coverage",
        "MIN_REQUIRED_MEASURED_SPEEDUP_CASES",
        "MIN_REQUIRED_SPEEDUP_CI_CASES",
        "paper output bundle must include at least one passed measured speedup case",
        "paper output bundle must include at least one passed speedup confidence interval case",
        "paper output bundle is missing measured speedup cases for required models",
        "paper output bundle is missing speedup confidence intervals for required models",
        "_require_scatter_svg_semantics",
        "_svg_element_count",
        "_svg_element_count_with_attribute",
        "_svg_case_values_for_role",
        "speedup_svg data-case markers must match passed measured-speedup cases",
        "speedup_uncertainty_svg CI data-case markers must match passed CI cases",
        "data-case markers must match passed summary data pairs",
        "write_speedup_uncertainty_svg",
        "SPEEDUP_UNCERTAINTY_TITLE",
        'data-role="speedup-ci"',
        'data-role="speedup-bar"',
        'data-role="scatter-point"',
        "speedup_uncertainty.svg",
        "verify_output",
        "verifying output bundle",
        "DOWNLOAD_PROGRESS_INTERVAL_BYTES",
        "_update_download_progress",
        "_emit_download_progress",
        "download_progress",
        "EXTERNAL_DISABLED_COMMAND_LABEL",
        "EXTERNAL_ENABLED_COMMAND_LABEL",
        "PIPELINE_OUTPUT",
        '--verify-pipeline-report "$PIPELINE_OUTPUT"',
        "STAGE_REPORT_FINGERPRINTS_KEY",
        "OUTPUT_BUNDLE_VERIFICATION_KEY",
        "case_mode_controls",
        "MODE_CONTROLS_KEY",
        "COMMAND_ENV_SNAPSHOT_KEYS",
        "tracked_env",
        "ENV_FLAG_FALSE_VALUES",
        "ENV_FLAG_FALSE_VALUES_KEY",
        "_as_env_value",
        "env_flag_is_enabled",
        "ABLATION_MODE_CHOICES",
        "ABLATION_OVERRIDE_CASE_KINDS",
        "--ablation-mode-override",
        "_apply_ablation_mode_override",
        "_runtime_override_comment",
        "CLI runtime overrides",
        "runtime_overrides",
        "SLURM_REPO_ROOT_ENV_NAME",
        "SLURM_MANIFEST_PATH_ENV_NAME",
        "SLURM_OUTPUT_DIR_ENV_NAME",
        "SLURM_SCRIPT_VERIFICATION_SCHEMA_VERSION",
        "SLURM_SCRIPT_VERIFICATION_COMMENT",
        "SLURM_PYTHON_BIN_ENV_NAME",
        "SLURM_PYTHON_PROVENANCE_ENV_NAME",
        "SLURM_PYTHON_PROVENANCE_NAME",
        "SLURM_PYTHON_BIN_PROVENANCE_PREFIX",
        "SLURM_SUITE_RUNNER_PROVENANCE_PREFIX",
        "SLURM_PYTHON_VERSION_PROVENANCE_PREFIX",
        "SLURM_SYS_EXECUTABLE_PROVENANCE_PREFIX",
        "SLURM_PYTHON_PROVENANCE_FUNCTION_NAME",
        "SLURM_PYTHON_PROVENANCE_COMMENT",
        "SLURM_PYTHON_PROVENANCE_MARKER_PREFIXES",
        "SLURM_PYTHON_PROVENANCE_ARTIFACT_KEY",
        "slurm_python_provenance",
        "python_runtime_provenance.txt",
        "python_version_probe",
        "python_provenance_before_preflight",
        "python_provenance_file_nonempty",
        "python_provenance_gate_before_preflight",
        "python_provenance_gate_after_final_verification",
        "python_provenance_comment_echo",
        "python_provenance_comment_marker",
        "_require_slurm_python_provenance_artifact",
        "_require_optional_slurm_python_provenance_semantics",
        "verified_slurm_python_provenance_count",
        "SLURM launcher must recheck Python provenance after final verification",
        "SLURM_ABLATION_SWEEP_SCHEMA_VERSION",
        "SLURM_ABLATION_SWEEP_COMMENT",
        "SLURM_ABLATION_SWEEP_VERIFICATION_SCHEMA_VERSION",
        "SLURM_ABLATION_SWEEP_VERIFICATION_COMMENT",
        "SLURM_ABLATION_SWEEP_INDEX_NAME",
        "DEFAULT_SLURM_ABLATION_SWEEP_MODES",
        "verify_slurm_script",
        "write_slurm_ablation_sweep",
        "verify_slurm_ablation_sweep_index",
        "script_fingerprint",
        "sweep_dir",
        "--verify-slurm-script",
        "--write-slurm-ablation-sweep-dir",
        "--slurm-ablation-sweep-modes",
        "--verify-slurm-ablation-sweep-index",
        "SLURM_SUITE_RUNNER_RELATIVE_PATH",
        "--slurm-repo-root",
        "--slurm-manifest-path",
        "--slurm-output-dir",
        "SLURM repo root must not be empty",
        "SLURM manifest path must not be empty",
        "SLURM output dir must not be empty",
        'cd "${SLURM_REPO_ROOT_ENV_NAME}"',
        "_has_one_sided_ablation_case",
        "one-sided ablation suite",
        "TIMING_MODES_KEY",
        "_external_timing_modes_for_ablation",
        "_external_pair_command_requirement_errors",
        "_validate_one_sided_benchmark_report",
        "external_pair_final_paper_ablation_mode",
        "sevennet_final_paper_ablation_mode",
        "EVIDENCE_FINGERPRINTS_KEY",
        "evidence_fingerprints",
        "bundle_check.BUNDLE_REPORT_COMMENT",
        "_require_report_comment(",
        "MACE",
        "NequIP",
    ),
    "tools/run_isodelta_validation.py": (
        "VALIDATION_REPORT_SCHEMA_VERSION",
        "VALIDATION_REPORT_COMMENT",
        "--expected-branch",
        "elapsed_seconds",
        "stdout_tail",
        "stderr_tail",
        "test_isodelta_sync_gate.py",
        "sevenn/scripts/deploy.py",
    ),
    "tools/run_isodelta_lammps_benchmark.py": (
        "ABLATION_MODE_CHOICES",
        "--ablation-mode",
        "benchmark_cases_for_ablation_mode",
        "BENCHMARK_REPORT_COMMENT",
        "GENERATED_REPORT_COMMENT_KEY",
        '"benchmark_cases": [case.name for case in benchmark_cases]',
    ),
    "tools/check_isodelta_benchmark_report.py": (
        "BENCHMARK_REPORT_COMMENT",
        "GENERATED_REPORT_COMMENT_KEY",
        "_check_report_comment",
        "REQUIRED_CASE_ENVIRONMENT_OVERRIDES",
        "ENV_FLAG_FALSE_VALUES",
        "env_flag_is_enabled",
        "must be absent ",
        "or false for enabled case",
    ),
    "tools/check_isodelta_evidence_bundle.py": (
        "BUNDLE_REPORT_COMMENT",
        "GENERATED_REPORT_COMMENT_KEY",
        "GENERATED_REPORT_COMMENT_KEY: BUNDLE_REPORT_COMMENT",
        "BUNDLE_SCHEMA_VERSION_KEY",
        "collect_run_provenance",
    ),
    "tools/check_isodelta_mlip_trace.py": (
        "TRACE_ARTIFACT_COMMENT",
        "TRACE_EVIDENCE_REPORT_COMMENT",
        "GENERATED_ARTIFACT_COMMENT_KEY",
        "GENERATED_REPORT_COMMENT_KEY",
        "_check_report_comment",
        "must describe the trace evidence purpose",
        "GENERATED_REPORT_COMMENT_KEY: TRACE_EVIDENCE_REPORT_COMMENT",
    ),
    "tools/run_isodelta_mlip_trace_demo.py": (
        "SUMMARY_REPORT_COMMENT",
        "trace_check.TRACE_ARTIFACT_COMMENT",
        "trace_check.GENERATED_REPORT_COMMENT_KEY",
    ),
    "tools/run_isodelta_experiment.py": (
        "ABLATION_MODE_CHOICES",
        "--ablation-mode",
        "ablation-benchmark",
        "should_run_publishable_pair_gates",
        "min_speedup requires paired ablation_mode",
        "EXPERIMENT_REPORT_COMMENT",
        "GENERATED_REPORT_COMMENT_KEY",
        "GENERATED_REPORT_COMMENT_KEY: EXPERIMENT_REPORT_COMMENT",
        "FINGERPRINT_ALGORITHM",
        "FILE_FINGERPRINT_CHUNK_BYTES",
        "file_fingerprint",
        "EXPERIMENT_LOG_SIDECAR_SCHEMA_VERSION",
        "EXPERIMENT_LOG_SIDECAR_COMMENT",
        "COMMAND_LOG_SIDECAR_FINGERPRINT_KEY",
        "command_log_sidecar_path",
        "write_command_log_sidecar",
        "command_log_fingerprint",
        "stdout_fingerprint",
        "stderr_fingerprint",
    ),
    "tools/check_isodelta_experiment_report.py": (
        "EXPERIMENT_REPORT_CHECK_SCHEMA_VERSION",
        "EXPERIMENT_REPORT_CHECK_COMMENT",
        "validate_experiment_report",
        "_check_fingerprint",
        "_check_command_log_sidecar",
        "_write_evidence",
        "--output",
        "experiment_driver.command_log_fingerprint",
        "COMMAND_LOG_SIDECAR_FINGERPRINT_KEY",
        "SHA256_KEY",
    ),
    "tools/run_isodelta_sync_gate.py": (
        "SYNC_REPORT_SCHEMA_VERSION",
        "EXPECTED_VALIDATION_REPORT_SCHEMA_VERSION",
        "SYNC_REPORT_COMMENT",
        "EXPECTED_VALIDATION_REPORT_COMMENT",
        "PUSH_AUTH_ENVIRONMENT",
        "--expected-branch",
        "_sync_git_provenance",
        "git_provenance",
        "STATUS_REMOTE_VERIFICATION_FAILED",
        "REMOTE_REF_VERIFICATION_KEY",
        "REMOTE_REF_VERIFY_COMMAND_NAME",
        "VALIDATION_REPORT_FINGERPRINT_KEY",
        "VALIDATION_REPORT_SUMMARY_KEY",
        "SYNC_COMMAND_SUMMARY_KEY",
        "_sync_command_summary",
        "PUSH_BRANCH_PRECONDITION_COMMAND_NAME",
        "_branch_precondition_failure_record",
        "STATUS_VALIDATION_REPORT_MISSING",
        "STATUS_VALIDATION_REPORT_INVALID",
        "_validation_report_summary",
        "command_failure_count",
        "VALIDATION_REPORT_COMMAND_REQUIRED_FIELDS",
        "GIT_OBJECT_ID_HEX_LENGTHS",
        "_is_git_object_id",
        "validation report git_branch does not match push branch",
        "validation report git_commit is not a full Git object id",
        "validation report git_status_short is missing",
        "validation report git_status_short must be a string or null",
        "validation report git_status_short does not match sync worktree snapshot",
        "remote branch commit is not a full Git object id",
        "command_missing_field_count",
        "_validation_command_record_has_valid_shape",
        "command_invalid_field_count",
        "validation report commands are missing required fields",
        "validation report commands have invalid field values",
        "validation report commands include nonzero returncodes",
        "STATUS_DIRTY_WORKTREE",
        "STATUS_GIT_HEAD_UNAVAILABLE",
        "STATUS_TARGET_BRANCH_MISMATCH",
        "LOCAL_HEAD_PRECONDITION_KEY",
        "LOCAL_HEAD_PRECONDITION_COMMAND_NAME",
        "HEAD_COMMIT_COMMAND",
        "_local_head_precondition_report",
        "_local_head_precondition_failure_record",
        "TARGET_BRANCH_PRECONDITION_KEY",
        "TARGET_BRANCH_PRECONDITION_COMMAND_NAME",
        "_target_branch_commit_command",
        "_target_branch_precondition_report",
        "_target_branch_precondition_failure_record",
        "expected_commit = target_branch_commit",
        "WORKTREE_STATUS_KEY",
        "_parse_status_short",
        "_worktree_status",
        "--require-clean-worktree",
        "_file_fingerprint",
        "_remote_ref_verify_command",
        "_verify_remote_ref",
        "--push-failure-bundle",
        "_write_push_failure_bundle",
        "PUSH_FAILURE_BUNDLE_VERIFY_COMMAND_NAME",
        "_bundle_verify_command",
        "PUSH_FAILURE_BUNDLE_KEY",
        "PUSH_FAILURE_BUNDLE_REPORT_SCHEMA_VERSION",
        "PUSH_FAILURE_BUNDLE_REPORT_COMMENT",
        "PUSH_FAILURE_BUNDLE_REPORT_FINGERPRINT_KEY",
        "_push_failure_bundle_report_path",
        "_write_push_failure_bundle_sidecar",
        "sidecar_report_fingerprint",
        "BUNDLE_HASH_READ_CHUNK_BYTES",
        "_bundle_file_fingerprint",
        "verify_returncode",
        "hashlib.sha256",
        "STATUS_PUSH_FAILED",
        "PUSH_FAILURE_REASON_AUTH_PROMPT_DISABLED",
        "_classify_push_failure",
        '"push_failure": _classify_push_failure(push_record)',
    ),
    "tools/check_isodelta_goal_readiness.py": (
        "ISODELTA_PYTHON_GLOB_PATTERNS",
        "ISODELTA_PRODUCTION_GLOB_PATTERNS",
        "GOAL_READINESS_REPORT_COMMENT",
        "GENERATED_REPORT_COMMENT_KEY",
        "GENERATED_REPORT_COMMENT_KEY: GOAL_READINESS_REPORT_COMMENT",
        "FORBIDDEN_IMPLEMENTATION_MARKERS",
        "placeholder",
        "temporary implementation",
        "magic number",
        "hardcoded",
        "FORBIDDEN_FILE_SNIPPETS",
        "_audit_isodelta_python_headers",
        "_audit_forbidden_implementation_markers",
        "_audit_forbidden_file_snippets",
    ),
    ".github/workflows/isodelta-halo.yml": (
        "--report-path isodelta_validation_report.json",
        "actions/upload-artifact@v4",
    ),
    "docs/source/user_guide/isodelta_halo.md": (
        "heap-backed `std::vector` storage",
        "runtime-sized stack arrays",
        "compiler-specific variable-length arrays",
        "`NEIGHMASK` before the runtime reads",
        "canonical atom index",
        "tokenized without mutating",
        "deployed `num_species`",
        "single space-delimited",
        "without leading or trailing whitespace",
        "TorchScript artifacts expose the same metadata contract",
        "serial `e3gnn` pair style mirrors the parallel parser checks",
        "baseline smoke runs and parallel",
        "aligned on deployment validation",
        "--prepare-artifacts",
        "--preflight-only",
        "--pipeline",
        "--readiness-check",
        "--verify-output-bundle",
        "`report_comment`",
        "`commands` array",
        "Timing command records outside the requested repeat indices are rejected",
        "cannot hide extra disabled or enabled trials",
        "`command_log_fingerprints` array",
        "sidecar JSON",
        "`command_log_sidecar_schema_version`",
        "keeps the raw stream untouched",
        "nested `log_fingerprint`",
        "MACE/NequIP stdout/stderr logs and their",
        "semantic paper-artifact checks",
        'same cell values as `summary["cases"]`',
        '`summary["correlations"]`',
        "`preflight_environment_snapshot.json`",
        "before GPU time",
        "package-version map",
        "same preflight snapshot",
        "replacement file",
        "figures and correlations use only passed or reused cases",
        "`data-case` markers",
        "command_timing.csv",
        "repeat_timing.csv",
        "speedup_uncertainty.csv",
        "speedup_uncertainty.svg",
        "lower/upper error bars",
        "speedup-ci",
        "`speedup_uncertainty.csv`/`speedup_uncertainty.md` must",
        "paper-ready CI table",
        '`summary["commands"]`',
        "source timing evidence",
        '`ablation_mode = "paired"`',
        "--ablation-mode-override",
        "`suite.runtime_overrides`",
        "CLI runtime overrides",
        "one-sided ablation suite",
        "`--verify-output-bundle`",
        "repository root in `REPO_ROOT`",
        "`--verify-slurm-script`",
        "`--write-slurm-ablation-sweep-dir`",
        "`--slurm-ablation-sweep-modes`",
        "`--verify-slurm-ablation-sweep-index`",
        "`slurm_ablation_sweep_index.json`",
        "mode-specific",
        "referenced launcher",
        "launcher SHA-256 fingerprint",
        "manifest snapshot body",
        "recorded manifest provenance",
        "final `--verify-pipeline-report` or `--verify-output-bundle`",
        "using `--slurm-repo-root`",
        "`--slurm-manifest-path`",
        "`--slurm-output-dir`",
        "/scratch/icpp/SevenNet-main",
        "/scratch/icpp/isodelta_cluster_paper_runs",
        "changes into `REPO_ROOT`",
        "one-sided SevenNet",
        "external_pair ablation",
        "`baseline-disabled` needs `disabled_command`",
        "`isodelta-enabled` needs `enabled_command`",
        "boolean-style parsing",
        "Surrounding",
        "whitespace is ignored before parsing",
        "`SEVENN_ISODELTA_HALO_DISABLE=0`",
        "keeps the cache enabled",
        '`SEVENN_ISODELTA_HALO_DISABLE=" off "` keeps the cache enabled',
        "`SEVENN_ISODELTA_HALO_PROFILE=0` keeps profiling off",
        "truthy value",
        "empty string",
        "`env_flag_false_values`",
        "false-value parser list",
        "cache-enabled evidence",
        "matching the C++",
        "runtime parser",
        "--ablation-mode baseline-disabled",
        "--ablation-mode isodelta-enabled",
        "one-sided ablation",
        "`tools/*isodelta*.py`",
        "`tests/unit_tests/test_isodelta*.py`",
        "temporary-work",
        "`placeholder`",
        "`temporary implementation`",
        "`magic number`",
        "`hardcoded`",
        "normal suite run also reopens the completed",
        "bundle verification failure returns a nonzero exit code",
        "`git_provenance` fields",
        "`report_comment`",
        "`experiment_log_sidecar_schema_version`",
        "experiment command-log sidecar",
        "raw `log_fingerprint`",
        "raw stdout/stderr stream stays unmodified",
        "experiment log sidecar",
        "top-level `report_comment`",
        "expected trace evidence `report_comment`",
        "The checker also requires `report_comment`",
        "`readiness_report.json`",
        "`artifact_preparation_report.json`",
        "`preflight_report.json`",
        "`pipeline_report.json`",
        "stage reports",
        "`validation_report_fingerprint`",
        "`validation_report_summary`",
        "validation report `report_comment`",
        "The resulting `bundle_evidence.json` records a top-level `report_comment`",
        "`report_comment` is missing",
        "Demo raw trace files carry `artifact_comment`",
        "self-describing outside the repository",
        "both generated JSON files describe their evidence purpose",
        "`sync_command_summary`",
        "`push_branch_precondition`",
        "`validation_report_missing`",
        "`validation_report_invalid`",
        "expected schema version",
        "required validation command fields",
        "`git_branch`",
        "`git_status_short`",
        "pre-validation worktree snapshot",
        "valid types and values",
        "zero failed validation command return codes",
        "`worktree_status`",
        "`--require-clean-worktree`",
        "`dirty_worktree`",
        "`local_head_precondition`",
        "`git_head_unavailable`",
        "`target_branch_precondition`",
        "`target_branch_mismatch`",
        "prechecked target branch commit",
        "`git ls-remote --heads`",
        "`remote_ref_verification`",
        "`remote_verification_failed`",
        "full Git object id",
        "malformed",
        "`download_progress`",
        "byte-level download progress",
        "suite artifact SHA gate",
        "human-facing path index points to a different file",
        '`--verify-pipeline-report "$PIPELINE_OUTPUT"`',
        "publication-success gate",
        "re-runs the current output-bundle verifier",
        "output-bundle `summary_json` path",
        "SLURM Python provenance",
        "experiment-report-check evidence counts",
        "failed or planned `pipeline_report.json` exits",
        "embedded output-bundle verification block must also keep its",
        "nested publication gate remains self-describing",
        "full final-paper stage sequence",
        "non-skipped stage",
        "present report files",
        "manifest path, SHA-256, byte size",
        "readiness report, artifact-preparation report, preflight report, and run",
        "same suite name, manifest path, manifest SHA-256 digest",
        "trace thresholds, runtime overrides, and artifact SHA gate",
        "skipped artifact-preparation",
        "`stages[*].report_path`",
        "`stage_report_fingerprints[*].report.path`",
        "cannot drift away from the protected fingerprint record",
        "`run_suite`",
        "`verify_output_bundle`",
        "verified output bundle",
        "copied or stale summary",
        "summary's suite metadata",
        "manifest SHA-256",
        "artifact SHA gate",
        "pipeline suite record itself stores `require_artifact_sha256`",
        "artifact preparation report, run plan, and final summary",
        "pipeline `modes` as booleans",
        "`dry_run = true`",
        "`skip_gpu_check = true`",
        "`allow_gpu_mismatch = true`",
        "SevenNet/MACE/NequIP required-model scope",
        "well-formed manifest fingerprint",
        "Runtime overrides are limited to known keys",
        "`ablation_mode = \"paired\"`",
        "one-sided ablation override",
        "reopens the fingerprinted readiness report",
        "every final-paper readiness check",
        "present and passed",
        "reopens the fingerprinted artifact preparation report",
        "`dry_run = false`",
        "missing_required_artifacts",
        "required artifact SHA-256 digest",
        "prepared required artifact names, paths, and",
        "different dataset or checkpoint path than the one scheduled",
        "preflight report must also carry that same suite metadata",
        "contain matching required artifact",
        "the one preflight actually checked",
        "reopens the fingerprinted run plan report",
        "final-paper execution modes",
        "checks `gpu_check_planned`",
        "required model case plans",
        "`run_plan.paper_outputs`",
        "summary artifact index",
        "command/repeat timing",
        "manifest snapshot",
        "Generated paper artifacts are self-describing",
        "`artifact_comment` or `report_comment`",
        "embedded output-bundle verification result",
        "nested bundle-verification",
        "Every generated table must keep its explanatory comment",
        "expected `<desc>` description",
        "at least one finite measured speedup",
        "at least one finite speedup confidence interval",
        "every required model to contribute passed measured speedup",
        "speedup numbers only on a failed case",
        "figures and correlations use only passed or reused cases",
        "`data-case` markers",
        "required_model_coverage.csv",
        "required_model_coverage.md",
        "model coverage table",
        "passed/reused trace evidence count",
        "fingerprinted trace JSON through the MLIP trace checker",
        "attributing a MACE trace to a NequIP case",
        "The final `suite_evidence` block is recomputed",
        "distinct trace-model counts",
        "`suite_evidence.min_trace_count`",
        "`suite_evidence.min_distinct_trace_models`",
        "same threshold",
        "silently lower the suite thresholds",
        "stronger suite thresholds",
        "skipped a required model",
        "reopens the fingerprinted preflight report",
        "checks `gpu_check`",
        "requested GPU count was actually detected",
        "changing only",
        "including suite evidence counts",
        "run_isodelta_sync_gate.py",
        "`push_failure` object",
        "`--push-failure-bundle`",
        "`push_failure_bundle`",
        "`git bundle verify` `verify_returncode`",
        "sidecar JSON",
        "`sidecar_report_fingerprint`",
        "`report_comment`",
        "SHA-256 digest",
        "byte size",
        "`auth-prompt-disabled`",
    ),
    "tests/unit_tests/test_isodelta_cluster_paper_suite.py": (
        "test_prepare_artifacts_downloads_and_writes_audit_report",
        "test_download_artifact_prints_terminal_progress_when_requested",
        "test_preflight_only_downloads_artifacts_and_runs_case_checks",
        "test_pipeline_runs_all_paper_stages_and_verifies_bundle",
        "test_pipeline_stops_when_readiness_fails",
        "test_pipeline_stops_when_preflight_fails",
        "test_pipeline_reports_bundle_verification_failure",
        "test_verify_pipeline_report_rejects_failed_pipeline_status",
        "test_manifest_validation_rejects_model_scoped_artifact_mismatch",
        "test_readiness_check_rejects_required_model_without_artifact_scope",
        "required_model_artifact_scope",
        "stage_path_drift_error",
        "summary_stage_path_drift_error",
        "wrong_stage_summary.json",
        "summary_suite_drift_error",
        "summary_manifest_path_drift_error",
        "summary_manifest_size_drift_error",
        "readiness_manifest_path_drift_error",
        "artifact_manifest_size_drift_error",
        "plan_manifest_size_drift_error",
        "readiness_threshold_drift_error",
        "artifact_output_dir_drift_error",
        "plan_threshold_drift_error",
        "artifact_plan_name_drift_error",
        "artifact_plan_path_drift_error",
        "preflight_suite_drift_error",
        "preflight_artifact_path_drift_error",
        "bundle_verification_comment_drift_error",
        "summary_threshold_drift_error",
        "summary_artifact_sha_gate_drift_error",
        "PIPELINE_SUMMARY_SUITE_GPU_DRIFT",
        "test_verify_pipeline_report_requires_suite_artifact_sha_gate",
        "plan_output_drift_error",
        "wrong_case_summary.csv",
        "bundle_summary_path_drift_error",
        "wrong_summary.json",
        "verified_paper_output_count",
        "wrong_readiness_report.json",
        "test_verify_pipeline_report_rejects_dry_run_passed_mode",
        "test_verify_pipeline_report_rejects_skipped_gpu_check_mode",
        "test_verify_pipeline_report_rejects_allowed_gpu_mismatch_mode",
        "test_verify_pipeline_report_rejects_failed_readiness_check",
        "test_verify_pipeline_report_rejects_failed_artifact_preparation",
        "test_verify_pipeline_report_rejects_gpu_skipped_run_plan",
        "test_verify_output_bundle_rejects_missing_generated_file_comment",
        "test_verify_pipeline_report_rejects_preflight_skipped_gpu_check",
        "test_verify_pipeline_report_requires_final_paper_suite_models",
        "test_verify_pipeline_report_rejects_unsupported_runtime_override",
        "test_verify_pipeline_report_rejects_one_sided_runtime_override",
        "test_verify_pipeline_report_rejects_shallow_passed_report",
        "test_verify_pipeline_report_requires_success_stage_fingerprints",
        "test_verify_pipeline_report_rejects_absent_success_stage_fingerprint",
        "test_verify_pipeline_report_requires_passed_bundle_verification",
        "OUTPUT_BUNDLE_VERIFICATION_COUNT_KEYS",
        "bundle_count_drift_errors",
        "verified_suite_evidence_count",
        "verified_slurm_python_provenance_count",
        "verified_preflight_environment_snapshot_count",
        "verified_preflight_environment_snapshot_bundle_count",
        "verified_experiment_report_check_count",
        "test_verify_output_bundle_accepts_preflight_environment_snapshot_artifact",
        "test_verify_output_bundle_rejects_incomplete_preflight_environment_snapshot",
        "wrong_preflight_environment_snapshot.json",
        "test_manifest_validation_rejects_enabled_external_pair_disable_env",
        "test_manifest_validation_accepts_false_enabled_disable_env",
        "test_manifest_validation_accepts_empty_enabled_disable_env",
        "test_external_timing_report_rejects_missing_report_comment",
        "test_external_timing_report_rejects_mismatched_mode_controls",
        "test_external_timing_report_requires_repeat_command_records",
        "test_external_timing_report_rejects_unexpected_timing_command_record",
        "test_external_timing_report_rejects_failed_command_record",
        "test_external_timing_report_rejects_mutated_command_log",
        "test_command_records_include_cwd_and_tracked_environment",
        "test_verify_output_bundle_rejects_missing_command_log_sidecar",
        "test_verify_output_bundle_rejects_mismatched_command_fingerprints",
        "test_verify_output_bundle_rejects_mutated_source_evidence",
        "test_verify_output_bundle_accepts_slurm_python_provenance_artifact",
        "test_verify_output_bundle_rejects_uncommented_slurm_python_provenance_artifact",
        "test_verify_output_bundle_rejects_mutated_external_command_log",
        "test_verify_output_bundle_rejects_semantically_invalid_svg_artifact",
        "test_verify_output_bundle_rejects_missing_measured_speedup",
        "test_verify_output_bundle_rejects_missing_speedup_confidence_interval",
        "test_verify_output_bundle_rejects_missing_speedup_uncertainty_svg",
        "test_verify_output_bundle_rejects_missing_required_model_speedup",
        "test_verify_output_bundle_rejects_failed_required_model_speedup",
        "test_build_correlation_rows_ignores_failed_case_metrics",
        "test_verify_output_bundle_rejects_failed_case_speedup_svg_marker",
        "test_verify_output_bundle_rejects_missing_required_model_speedup_ci",
        "test_verify_output_bundle_rejects_required_model_coverage_value_drift",
        "test_verify_output_bundle_rejects_failed_trace_coverage_drift",
        "test_verify_output_bundle_rejects_trace_evidence_path_drift",
        "test_verify_output_bundle_rejects_invalid_fingerprinted_trace",
        "test_validate_case_outputs_rejects_missing_required_trace_model_label",
        "test_verify_output_bundle_rejects_required_trace_model_label_drift",
        "test_verify_output_bundle_requires_suite_evidence_for_required_models",
        "test_verify_output_bundle_rejects_suite_evidence_trace_count_drift",
        "test_verify_output_bundle_rejects_missing_suite_evidence_threshold_origin",
        "test_verify_output_bundle_rejects_suite_evidence_min_trace_origin_drift",
        "test_verify_output_bundle_rejects_suite_evidence_min_trace_drift",
        "test_verify_output_bundle_rejects_suite_evidence_min_model_origin_drift",
        "test_verify_output_bundle_rejects_suite_evidence_min_model_drift",
        "test_verify_output_bundle_rejects_speedup_svg_label_drift",
        "test_verify_output_bundle_rejects_speedup_uncertainty_svg_point_drift",
        "test_verify_output_bundle_rejects_scatter_svg_point_count_drift",
        "test_verify_output_bundle_rejects_manifest_snapshot_body_drift",
        "test_verify_output_bundle_rejects_mismatched_artifact_index_path",
        "test_verify_output_bundle_rejects_case_summary_value_drift",
        "test_verify_output_bundle_rejects_correlation_value_drift",
        "test_verify_output_bundle_rejects_summary_correlation_drift",
        "test_verify_output_bundle_rejects_command_timing_value_drift",
        "test_verify_output_bundle_rejects_repeat_timing_value_drift",
        "test_verify_output_bundle_rejects_speedup_uncertainty_drift",
        "test_run_suite_verifies_output_bundle_after_writing",
        "test_write_slurm_script_rejects_collect_only_pipeline_launcher",
        "test_readiness_check_accepts_strict_three_model_paired_manifest",
        "test_sevennet_manifest_ablation_mode_reaches_experiment_driver",
        "test_one_sided_sevennet_benchmark_report_validates_as_raw_ablation",
        "test_validate_case_outputs_rejects_uncommented_bundle_evidence",
        "test_validate_case_outputs_rejects_wrong_bundle_comment",
        "test_one_sided_sevennet_benchmark_rejects_speedup_claim",
        "test_readiness_check_rejects_one_sided_sevennet_ablation",
        "test_external_pair_ablation_mode_runs_one_command_side",
        "test_external_pair_enabled_ablation_omits_disabled_command",
        "test_external_pair_paired_mode_still_requires_both_commands",
        "test_cli_ablation_override_updates_runtime_timing_cases",
        "test_cli_ablation_override_is_recorded_in_run_plan",
        "# CLI runtime overrides: ablation_mode=isodelta-enabled.",
        "test_cli_ablation_override_requires_runtime_timing_case",
        "test_write_slurm_script_uses_ablation_override_without_pipeline",
        'cd "$REPO_ROOT"',
        "$REPO_ROOT/tools/run_isodelta_cluster_paper_suite.py",
        "/scratch/icpp/SevenNet-main",
        "/scratch/icpp/SevenNet-main/isodelta_cluster_suite.toml",
        "ISODELTA_OUTPUT_DIR='/scratch/icpp/paper outputs'",
        'COMMON_ARGS+=(--output-dir "${ISODELTA_OUTPUT_DIR}")',
        'PYTHON_PROVENANCE_OUTPUT="${ISODELTA_OUTPUT_DIR}/python_runtime_provenance.txt"',
        "verify_python_provenance() {",
        "provenance_call = \"\\nverify_python_provenance\\n\"",
        "IsoDelta-Halo Python runtime provenance captured by the generated SLURM launcher",
        'echo "PYTHON_BIN=$PYTHON_BIN"',
        '"$PYTHON_BIN" --version 2>&1',
        'test -s "$PYTHON_PROVENANCE_OUTPUT"',
        r"grep -q '^PYTHON_BIN=' \"$PYTHON_PROVENANCE_OUTPUT\"",
        r"grep -q '^Python ' \"$PYTHON_PROVENANCE_OUTPUT\"",
        "self.assertNotIn(str(root), script)",
        "test_verify_slurm_script_rejects_missing_python_provenance",
        "test_verify_slurm_script_rejects_missing_python_provenance_gate",
        "test_verify_slurm_script_rejects_missing_python_provenance_comment",
        "test_verify_slurm_script_rejects_missing_final_python_provenance_gate",
        "test_verify_slurm_script_rejects_missing_final_gate",
        "pipeline-report or output-bundle verification",
        "test_write_slurm_ablation_sweep_creates_verified_mode_launchers",
        "test_verify_slurm_ablation_sweep_rejects_launcher_mode_drift",
        "test_write_slurm_ablation_sweep_rejects_single_mode_override",
        "slurm_ablation_sweep_index.json",
        "SHA-256 mismatch",
        "test_external_pair_one_sided_timing_validates_without_speedup",
        "test_external_pair_one_sided_timing_rejects_speedup_claim",
        "test_readiness_check_rejects_one_sided_external_pair_ablation",
        "test_sevennet_case_runs_experiment_report_checker",
        "test_validate_case_outputs_accepts_experiment_report_check",
        "test_summary_verifier_counts_experiment_report_check_evidence",
        "test_run_suite_verifies_sevennet_experiment_check_evidence",
    ),
    "tests/unit_tests/test_isodelta_benchmark_report_check.py": (
        "test_validate_report_accepts_false_disable_env_in_enabled_case",
        "test_validate_report_rejects_missing_report_comment",
        "test_validate_report_rejects_wrong_report_comment",
    ),
    "tests/unit_tests/test_isodelta_build_prereqs.py": (
        "EXPECTED_PREREQ_REPORT_SCHEMA_VERSION",
        "EXPECTED_PREREQ_REPORT_COMMENT",
        "test_build_prereq_report_carries_report_comment",
        'report["report_schema_version"]',
        'report["report_comment"]',
    ),
    "tests/unit_tests/test_isodelta_lammps_binary_check.py": (
        "EXPECTED_BINARY_SMOKE_REPORT_SCHEMA_VERSION",
        "EXPECTED_BINARY_SMOKE_REPORT_COMMENT",
        "test_build_binary_check_report_carries_report_comment",
        'report["report_schema_version"]',
        'report["report_comment"]',
    ),
    "tests/unit_tests/test_isodelta_evidence_bundle_check.py": (
        "EXPECTED_BUNDLE_REPORT_COMMENT",
        'evidence["report_comment"]',
        'written["report_comment"]',
    ),
    "tests/unit_tests/test_isodelta_mlip_trace_check.py": (
        "EXPECTED_TRACE_EVIDENCE_REPORT_COMMENT",
        "test_validate_trace_rejects_missing_report_comment",
        "test_validate_trace_rejects_wrong_report_comment",
        'written["report_comment"]',
    ),
    "tests/unit_tests/test_isodelta_mlip_trace_demo.py": (
        "EXPECTED_TRACE_ARTIFACT_COMMENT",
        "EXPECTED_SUMMARY_REPORT_COMMENT",
        'trace["artifact_comment"]',
        'persisted_summary["report_comment"]',
    ),
    "tests/unit_tests/test_isodelta_benchmark_parser.py": (
        "test_ablation_mode_selects_one_benchmark_case",
        "test_ablation_only_summary_has_no_speedup_claim",
        "test_validate_benchmark_options_rejects_unknown_ablation_mode",
    ),
    "tests/unit_tests/test_isodelta_experiment_runner.py": (
        "test_build_experiment_commands_supports_one_sided_ablation",
        "test_validate_config_rejects_speedup_gate_for_one_sided_ablation",
        "EXPECTED_EXPERIMENT_REPORT_COMMENT",
        "EXPECTED_FINGERPRINT_ALGORITHM",
        "EXPECTED_EXPERIMENT_LOG_SIDECAR_SCHEMA_VERSION",
        "EMPTY_SHA256_HEXDIGEST",
        'report["report_comment"]',
        'command["stdout_fingerprint"]',
        'command["stderr_fingerprint"]',
        "sidecar_fingerprint",
        "experiment_log_sidecar_schema_version",
    ),
    "tests/unit_tests/test_isodelta_experiment_report_check.py": (
        "EXPECTED_CHECK_REPORT_COMMENT",
        "test_validate_experiment_report_accepts_matching_log_fingerprints",
        "test_validate_experiment_report_rejects_missing_report_comment",
        "test_validate_experiment_report_rejects_changed_stdout_log",
        "test_validate_experiment_report_rejects_missing_stdout_sidecar",
        "test_main_writes_output_evidence_file",
        "test_main_writes_failure_output_evidence_file",
        "stdout_fingerprint.sha256",
    ),
    "tests/unit_tests/test_isodelta_sync_gate.py": (
        "IsoDeltaSyncGateTest",
        "test_validation_command_enforces_target_branch",
        "test_run_sync_records_git_provenance_for_push_target",
        "test_run_sync_fails_when_remote_ref_does_not_match",
        "test_run_sync_fails_when_remote_ref_commit_is_malformed",
        "test_run_sync_rejects_validation_report_malformed_commit",
        "test_run_sync_rejects_validation_report_wrong_git_branch",
        "test_run_sync_rejects_validation_report_without_git_status",
        "test_run_sync_rejects_validation_report_invalid_git_status",
        "test_run_sync_rejects_validation_report_changed_git_status_snapshot",
        "test_run_sync_requires_full_report_commit_without_local_head",
        "test_run_sync_rejects_push_when_local_head_is_unavailable",
        "test_run_sync_rejects_push_when_target_branch_differs_from_head",
        "test_remote_verification_uses_prechecked_target_commit",
        "test_run_sync_rejects_missing_validation_report",
        "test_run_sync_rejects_failed_validation_report_json",
        "test_run_sync_rejects_failed_validation_report_command",
        "test_run_sync_rejects_validation_report_without_comment",
        "test_run_sync_can_require_clean_worktree",
        "WORKTREE_STATUS_KEY",
        "VALIDATION_REPORT_FINGERPRINT_KEY",
        "VALIDATION_REPORT_SUMMARY_KEY",
        "REMOTE_REF_VERIFY_COMMAND_NAME",
        "SYNC_COMMAND_REQUIRED_FIELDS",
        "assert_sync_command_record_shape",
        "test_run_sync_records_replayable_branch_precondition_failure",
        "test_run_sync_can_write_bundle_after_push_failure",
        "PUSH_FAILURE_BUNDLE_VERIFY_COMMAND_NAME",
        "PUSH_FAILURE_BUNDLE_REPORT_SCHEMA_VERSION",
        "PUSH_FAILURE_BUNDLE_REPORT_COMMENT",
        "PUSH_FAILURE_BUNDLE_REPORT_FINGERPRINT_KEY",
        "sidecar_report_path",
        "hashlib.sha256(b\"bundle\").hexdigest()",
        "test_run_sync_classifies_noninteractive_auth_push_failure",
        "STATUS_VALIDATION_FAILED",
    ),
    "tests/unit_tests/test_isodelta_validation_runner.py": (
        "test_expected_branch_reaches_goal_readiness_command",
        "test_expected_branch_does_not_reach_py_compile",
        "VALIDATION_COMMAND_REQUIRED_FIELDS",
        "test_run_validation_writes_sync_gate_compatible_command_record",
        "codex/isodelta-halo-runtime",
    ),
    "tests/unit_tests/test_isodelta_goal_readiness.py": (
        "test_expected_branch_names_codex_work_branch",
        "test_goal_readiness_report_carries_report_comment",
        "GOAL_READINESS_REPORT_COMMENT",
        "GENERATED_REPORT_COMMENT_KEY",
        "test_goal_readiness_accepts_isodelta_python_headers",
        "test_goal_readiness_rejects_isodelta_python_without_header",
        "test_goal_readiness_rejects_forbidden_production_marker",
        "test_goal_readiness_rejects_explicit_temporary_implementation_markers",
        "test_goal_readiness_ignores_marker_string_literals",
        "test_goal_readiness_rejects_forbidden_file_snippet",
    ),
}
COMMENT_PREFIX_REQUIREMENTS = {
    ".github/workflows/isodelta-halo.yml": "#",
    "docs/source/user_guide/isodelta_halo.md": "<!--",
    "tools/check_isodelta_goal_readiness.py": '"""',
    "tools/run_isodelta_cluster_paper_suite.py": '"""',
    "tools/run_isodelta_sync_gate.py": '"""',
    "tools/run_isodelta_validation.py": '"""',
    "tests/unit_tests/test_isodelta_sync_gate.py": '"""',
}


def _read_text(path: Path) -> str:
    """Read one source file with the repository-wide encoding."""
    return path.read_text(encoding="utf-8")


def _check_record(name: str, passed: bool, detail: str) -> dict[str, Any]:
    """Return one JSON-friendly audit check record."""
    return {"name": name, "passed": passed, "detail": detail}


def _git_metadata(root: Path, command: tuple[str, ...]) -> str | None:
    """Collect short git metadata without making the audit depend on git state."""
    try:
        completed = subprocess.run(
            command,
            cwd=root,
            text=True,
            capture_output=True,
            check=False,
            timeout=GIT_METADATA_TIMEOUT_SECONDS,
        )
    except (OSError, subprocess.SubprocessError):
        return None
    if completed.returncode != SUCCESS_RETURN_CODE:
        return None
    return completed.stdout.strip()


def _audit_required_snippets(
    root: Path,
    required_file_snippets: dict[str, tuple[str, ...]],
) -> list[dict[str, Any]]:
    """Check that core files exist and still expose required feature markers."""
    records: list[dict[str, Any]] = []
    for relative_path, snippets in required_file_snippets.items():
        path = root / relative_path
        if not path.exists():
            records.append(_check_record(f"required_file:{relative_path}", False, "missing file"))
            continue
        text = _read_text(path)
        missing = [snippet for snippet in snippets if snippet not in text]
        records.append(
            _check_record(
                f"required_file:{relative_path}",
                not missing,
                "missing snippets: " + ", ".join(missing) if missing else "all snippets present",
            )
        )
    return records


def _audit_comment_prefixes(
    root: Path,
    comment_prefix_requirements: dict[str, str],
) -> list[dict[str, Any]]:
    """Check that generated or added documentation/script files explain themselves."""
    records: list[dict[str, Any]] = []
    for relative_path, prefix in comment_prefix_requirements.items():
        path = root / relative_path
        if not path.exists():
            records.append(_check_record(f"comment_prefix:{relative_path}", False, "missing file"))
            continue
        stripped_text = _read_text(path).lstrip()
        records.append(
            _check_record(
                f"comment_prefix:{relative_path}",
                stripped_text.startswith(prefix),
                f"expected prefix {prefix!r}",
            )
        )
    return records


def _audit_forbidden_file_snippets(
    root: Path,
    forbidden_file_snippets: dict[str, tuple[str, ...]],
) -> list[dict[str, Any]]:
    """Reject known magic-number or brittle implementation snippets."""
    records: list[dict[str, Any]] = []
    for relative_path, snippets in forbidden_file_snippets.items():
        path = root / relative_path
        if not path.exists():
            records.append(
                _check_record(
                    f"forbidden_file:{relative_path}",
                    False,
                    "missing file",
                )
            )
            continue
        text = _read_text(path)
        present = [snippet for snippet in snippets if snippet in text]
        detail = (
            "no forbidden snippets"
            if not present
            else "present snippets: " + ", ".join(present)
        )
        records.append(
            _check_record(
                f"forbidden_file:{relative_path}",
                not present,
                detail,
            )
        )
    return records


def _audit_isodelta_python_headers(
    root: Path,
    glob_patterns: tuple[str, ...] = ISODELTA_PYTHON_GLOB_PATTERNS,
) -> list[dict[str, Any]]:
    """Require every IsoDelta Python tool or test to start with a file comment."""
    records: list[dict[str, Any]] = []
    seen_paths: set[Path] = set()
    for glob_pattern in glob_patterns:
        for path in sorted(root.glob(glob_pattern)):
            if not path.is_file() or path in seen_paths:
                continue
            seen_paths.add(path)
            relative_path = path.relative_to(root).as_posix()
            stripped_text = _read_text(path).lstrip()
            records.append(
                _check_record(
                    f"isodelta_python_header:{relative_path}",
                    stripped_text.startswith(PYTHON_HEADER_PREFIX),
                    f"expected prefix {PYTHON_HEADER_PREFIX!r}",
                )
            )
    return records


def _contains_marker(text: str, markers: tuple[str, ...]) -> str | None:
    """Return the first forbidden marker found in a piece of source text."""
    lowered_text = text.lower()
    for marker in markers:
        if marker.lower() in lowered_text:
            return marker
    return None


def _python_comment_and_docstring_text(path: Path) -> list[tuple[int, str]]:
    """Return Python comments and docstrings while ignoring ordinary literals."""
    source_text = _read_text(path)
    evidence: list[tuple[int, str]] = []
    tokens = tokenize.generate_tokens(io.StringIO(source_text).readline)
    for token in tokens:
        if token.type == tokenize.COMMENT:
            evidence.append((token.start[0], token.string))
    try:
        module = ast.parse(source_text, filename=str(path))
    except SyntaxError:
        return evidence
    candidate_nodes: list[ast.AST] = [module]
    candidate_nodes.extend(
        node
        for node in ast.walk(module)
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef))
    )
    for node in candidate_nodes:
        body = getattr(node, "body", [])
        if not body:
            continue
        first_statement = body[0]
        if (
            isinstance(first_statement, ast.Expr)
            and isinstance(first_statement.value, ast.Constant)
            and isinstance(first_statement.value.value, str)
        ):
            evidence.append((first_statement.lineno, first_statement.value.value))
    return evidence


def _forbidden_marker_hits(path: Path, markers: tuple[str, ...]) -> list[str]:
    """Find implementation markers in comments and other human-authored notes."""
    suffix = path.suffix.lower()
    if suffix == ".py":
        searchable_text = _python_comment_and_docstring_text(path)
    else:
        searchable_text = [
            (line_number, line)
            for line_number, line in enumerate(_read_text(path).splitlines(), start=1)
        ]
    hits: list[str] = []
    for line_number, text in searchable_text:
        marker = _contains_marker(text, markers)
        if marker is not None:
            hits.append(f"line {line_number}: {marker}")
    return hits


def _audit_forbidden_implementation_markers(
    root: Path,
    glob_patterns: tuple[str, ...] = ISODELTA_PRODUCTION_GLOB_PATTERNS,
    markers: tuple[str, ...] = FORBIDDEN_IMPLEMENTATION_MARKERS,
) -> list[dict[str, Any]]:
    """Reject temporary-work markers in production IsoDelta implementation files."""
    records: list[dict[str, Any]] = []
    seen_paths: set[Path] = set()
    for glob_pattern in glob_patterns:
        for path in sorted(root.glob(glob_pattern)):
            if not path.is_file() or path in seen_paths:
                continue
            seen_paths.add(path)
            relative_path = path.relative_to(root).as_posix()
            hits = _forbidden_marker_hits(path, markers)
            records.append(
                _check_record(
                    f"forbidden_implementation_marker:{relative_path}",
                    not hits,
                    "no forbidden markers" if not hits else "; ".join(hits),
                )
            )
    return records


def build_goal_readiness_report(
    *,
    root: Path = REPO_ROOT,
    expected_branch: str | None = None,
    required_file_snippets: dict[str, tuple[str, ...]] | None = None,
    forbidden_file_snippets: dict[str, tuple[str, ...]] | None = None,
    comment_prefix_requirements: dict[str, str] | None = None,
    isodelta_python_glob_patterns: tuple[str, ...] = ISODELTA_PYTHON_GLOB_PATTERNS,
    implementation_marker_glob_patterns: tuple[str, ...] = (
        ISODELTA_PRODUCTION_GLOB_PATTERNS
    ),
) -> dict[str, Any]:
    """Build a report proving local source readiness for the active goal."""
    snippet_requirements = (
        REQUIRED_FILE_SNIPPETS
        if required_file_snippets is None
        else required_file_snippets
    )
    prefix_requirements = (
        COMMENT_PREFIX_REQUIREMENTS
        if comment_prefix_requirements is None
        else comment_prefix_requirements
    )
    forbidden_snippets = (
        FORBIDDEN_FILE_SNIPPETS
        if forbidden_file_snippets is None
        else forbidden_file_snippets
    )
    branch = _git_metadata(root, ("git", "branch", "--show-current"))
    checks = []
    if expected_branch is None:
        checks.append(_check_record("branch", True, STATUS_NOT_ENFORCED))
    else:
        checks.append(
            _check_record(
                "branch",
                branch == expected_branch,
                f"observed={branch!r}, expected={expected_branch!r}",
            )
        )
    checks.extend(_audit_required_snippets(root, snippet_requirements))
    checks.extend(_audit_forbidden_file_snippets(root, forbidden_snippets))
    checks.extend(_audit_comment_prefixes(root, prefix_requirements))
    checks.extend(_audit_isodelta_python_headers(root, isodelta_python_glob_patterns))
    checks.extend(
        _audit_forbidden_implementation_markers(
            root,
            implementation_marker_glob_patterns,
        )
    )
    status = STATUS_PASSED if all(record["passed"] for record in checks) else STATUS_FAILED
    return {
        "goal_readiness_schema_version": GOAL_READINESS_SCHEMA_VERSION,
        GENERATED_REPORT_COMMENT_KEY: GOAL_READINESS_REPORT_COMMENT,
        "status": status,
        "generated_at": time.strftime("%Y-%m-%dT%H:%M:%S%z"),
        "root": str(root),
        "expected_branch": expected_branch,
        "git_branch": branch,
        "git_commit": _git_metadata(root, ("git", "rev-parse", "HEAD")),
        "git_status_short": _git_metadata(root, ("git", "status", "--short")),
        "checks": checks,
    }


def _write_report(report_path: Path, payload: dict[str, Any]) -> None:
    """Persist a goal-readiness report for later completion audits."""
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse command-line options for the readiness audit."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--report-path",
        type=Path,
        default=None,
        help="Optional JSON report path for local goal-readiness evidence",
    )
    parser.add_argument(
        "--expected-branch",
        default=None,
        help="Optionally enforce an expected working branch",
    )
    return parser.parse_args(argv)


def main(argv: list[str] | None = None) -> int:
    """Run the local goal-readiness audit."""
    args = parse_args(argv)
    report = build_goal_readiness_report(expected_branch=args.expected_branch)
    if args.report_path is not None:
        _write_report(args.report_path, report)
    print(
        json.dumps(
            {
                "goal_readiness_report": str(args.report_path) if args.report_path is not None else None,
                "status": report["status"],
            },
            indent=2,
        )
    )
    return SUCCESS_RETURN_CODE if report["status"] == STATUS_PASSED else FAILURE_RETURN_CODE


if __name__ == "__main__":
    raise SystemExit(main())
